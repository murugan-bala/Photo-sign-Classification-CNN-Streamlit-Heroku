...
st.title("""
          Photo or Signature classification Prediction : sunglasses
         """
         )
...
def import_and_predict(image_data, model):
        ...
        img_array = np.array(image_data.resize((IMG_SHAPE,IMG_SHAPE)))
...
def import_and_predict_old(image_data, model):
        #print("Inside predict")
...
if file is None:
    ...
    if st.button("process"):
        ...
        result = import_and_predict(imagee, model)   # result = import_and_predict(imagee, model)---old function
        if result[0][0] == 1:
            prediction = 'photo'
            print(prediction)
            #st.write("It is a photo!.")
            st.markdown('**It is a photo!**.')
        else:
